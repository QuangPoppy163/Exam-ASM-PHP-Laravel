<!doctype html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport"
          content="width=device-width, user-scalable=no, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0">
    <meta http-equiv="X-UA-Compatible" content="ie=edge">
    <link rel="Milk Tea" href="{{{ asset('favicon.ico') }}}">
    <title>Cups Of Tea</title>
</head>
<body>
    <h1>Insert Information Milk Tea</h1>
    <form action="/admin/product/update" method="POST">
        <input type="hidden" name="id" value="{{$product_in_view -> id}}">
        {{csrf_field()}}
        <div>
            <label>Name</label>
            <input type="text" name="name" value="{{$product_in_view -> name}}">
        </div>
        <div>
            <label>Price</label>
            <input type="text" name="price" value="{{$product_in_view -> price}}">
        </div>
        <div>
            <label>Catogory</label>
            <select name="categoryId">
                <option value="1" {{$product_in_view -> categoryId == 1 ? 'selected' : ''}}>Tra Sua</option>
                <option value="2" {{$product_in_view -> categoryId == 2 ? 'selected' : ''}}>Tra Kem Cheese</option>
                <option value="3" {{$product_in_view -> categoryId == 3 ? 'selected' : ''}}>Tra Matcha</option>
                <option value="4" {{$product_in_view -> categoryId == 4 ? 'selected' : ''}}>Tra Hoa Qua</option>
            </select>
        </div>
        <div>
            <label>Description</label>
            <input type="text" name="description" value="{{$product_in_view -> description}}">
        </div>
        <div>
            <label>Image</label>
            <input type="text" name="image" value="{{$product_in_view -> image}}">
            <img src="{{$product_in_view -> image}}" alt="" style="width:200px;">
        </div>
        <div>
            <input type="submit" value="Submit">
            <input type="reset" value="Reset">
        </div>
    </form>
</body>
</html>